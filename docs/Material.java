...
public abstract class Material {
	...
	public Color shade(RayHit rayHit, List<Light> lights, List<Shape> objects) {
		...
		Vector3 normal = rayHit.getShape().getMaterial().getNormal(hitPoint); // Get object surface normal vector
		...
		for (Light light : lights) {
			...
			if (!inShadow(shadowRay, light, objects)) {
				...
				if (reflectiveness > 0) {
					...
					colors.add(baseColor.interpolate(shadedColor, 1 - reflectiveness));
				}
				else
	...
	private Color calculateReflection(Ray ray, Light light, List<Shape> objects, int depth) {
		...
		if (hit != null) { // TODO WAAAAAAY TO HEAVY
			...
			Vector3 normal = hit.getShape().getMaterial().getNormal(hitPoint);
