...
public class Ray {
    ...
    public RayHit cast(List<Object3D> objects) {
        RayHit hit = null;
        for (Iterator<Object3D> iterator = objects.iterator(); iterator.hasNext();) {
            ...
            // Shape shape = new Sphere(object3d.getAnchor(),
            // new BasicMaterial(new BloomShader(), ((Light) object3d).getColor()),
            // ((Light) object3d).getIntensity() / 10);
            // Vector3 hitPoint = shape.getIntersectionPoint(this);
            // if (hitPoint != null) {
            // RayHit hit2 = new RayHit(this, object3d, shape, hitPoint);
            // if (hit == null) {
            // hit = hit2;
            // }
            // else if (getOrigin().distance(hit.getHitPoint()) >
            // getOrigin().distance(hitPoint)) {
            // hit = hit2;
            // }
            // }
            // }
